...
class Event
{
    ...
    {
        if (func_num_args() > 2)
            self::$_hookBind[$event] = [$callback, $params];
        else if (func_num_args() > 1)
            self::$_hookBind[$event] = [$callback];
        else
        ...
        if ($handlers = self::$_hookBind[$event]) {
            ...
            if($params){
                if (method_exists(__CLASS__, $handlers[0]))
                    call_user_func_array([__CLASS__, $handlers[0]], $params);
                ...
                if (method_exists(__CLASS__, $handlers[0]))
                    call_user_func([__CLASS__, $handlers[0]]);
                else if (is_callable($handlers[0]))
    ...
    public static function filterTrigger($filterName, $args)
    {
        ...
        {
            if (is_string($args))
                call_user_func([__CLASS__, self::$_filterBind[$filterName]['callable']], $args);
            else if (is_array($args) AND self::$_filterBind[$filterName]['args'] == sizeof($args))
                call_user_func_array([__CLASS__, self::$_filterBind[$filterName]['callable']], $args);
        }
        else
        {
            if (Rec::$debug)
